...
class HistoryItemParser(HTMLParser):
    ...
    def handle_starttag(self,tag,attrs):
        ...
        elif tag == "span" and len(attrs) > 0 and attrs[0] == ('class', 'character'):
...
class DataHandler():
    ...
    def loadAllSearchItemsFromXml(self):
        ...
        searchData = searchResultXmlLoader.loadAllXmlFiles()
        return
...
class SearchResultXmlLoader():
    ...
    def loadXmlFile(self, file):
        ...
        items = searchResult.getElementsByTagName("Item")
        for item in items:
            try:
...
class SearchResultXmlGenerator():
    ...
    def writeToFile(self):
        f= open("data/" + self.searchItem.name.decode("utf-8").encode("gbk") + "_" + self.searchItem.category.decode("utf-8").encode("gbk") + ".xml", 'w')
...
if __name__ == '__main__':
    ...
    while quitCommand == False:
        ...
        if parameters[0] == "q" or parameters[0] == "quit" or parameters[0] == "exit":
            ...
            def add(parameters):
                alias = parameters[1]
                name = parameters[2].decode("gbk").encode("utf-8")
                nameU = parameters[2].decode("gbk")
                category = parameters[3].decode("gbk").encode("utf-8")
                categoryU = parameters[3].decode("gbk")
                if cmp(categoryU, u'散票') != 0 and cmp(categoryU, u'纪特邮票') != 0 and cmp(categoryU, u'文革邮票') != 0 and cmp(categoryU, u'编号邮票') != 0 and cmp(categoryU, u'JT邮票') != 0:
            ...
            if len(parameters) == 4:
                ...
                while True:
                    ...
                    if not l:
                        break
                    #ll = l.decode("utf-8").encode("gbk")
                    param = l.strip().split(' ', 3)
                    if len(param) == 4:
                        add(param)
                f.close()
            else:
